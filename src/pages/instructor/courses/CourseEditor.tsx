    ...
    validatePublishMutation.mutate(courseId!, {
      onSuccess: (result) => {
        ...
        submitReviewMutation.mutate(courseId!, {
          ...
          onError: (err: Error) => toast.error(err.message, err.message.includes('Hồ sơ người giảng dạy') ? { action: { label: 'Cập nhật hồ sơ', onClick: () => navigate('/account/settings/profile') } } : undefined),
        });
      },
