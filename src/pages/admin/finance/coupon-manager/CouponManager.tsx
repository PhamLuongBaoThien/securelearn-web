...
const getCouponStatus = (coupon: Coupon): CouponComputedStatus =>
...
const formatActor = (coupon: Coupon) => {
  ...
  if (coupon.createdBy)
    return (
      "ID: " +
      coupon.createdBy.slice(0, 8) +
      (coupon.createdBy.length > 8 ? "..." : "")
    );
  return "Chưa rõ";
};
...
export const CouponManager = () => {
  ...
  const redemptionsQuery = useAdminCouponDetailRedemptions(
    historyCoupon?._id || null,
    1,
    10,
  );
  ...
  const isUpdatingStatus =
    ...
    multiStatusMutation.isPending ||
    multiDeleteMutation.isPending;
  ...
  const validateForm = (): boolean => {
    ...
    if (!/^[A-Z0-9]+$/.test(code))
      return (
        toast.error("Mã coupon chỉ được chứa chữ cái viết hoa và chữ số."),
        false
      );
    if (!form.name.trim())
      return (toast.error("Tên hiển thị không được để trống."), false);
    if (form.value === undefined || form.value === null || isNaN(form.value))
      return (toast.error("Giá trị giảm giá không hợp lệ."), false);
    if (form.type === "PERCENT" && (form.value < 0 || form.value > 100))
      return (
        toast.error("Giá trị phần trăm phải nằm trong khoảng 0-100%."),
        false
      );
    if (form.type === "FIXED" && form.value <= 0)
      return (toast.error("Giá trị giảm cố định phải lớn hơn 0."), false);
    if (form.maxDiscountAmount !== null && form.maxDiscountAmount < 0)
      return (toast.error("Số tiền giảm tối đa không được nhỏ hơn 0."), false);
    if (form.minOrderAmount < 0)
      return (toast.error("Đơn hàng tối thiểu không được nhỏ hơn 0."), false);
    ...
    if (
      ...
      return (toast.error("Ngày kết thúc phải sau ngày bắt đầu."), false);
    return true;
  };
  ...
  const submit = (event: FormEvent) => {
    ...
    saveMutation.mutate(
      {
        id: editing?._id,
        payload: { ...form, code: form.code.trim().toUpperCase() },
      },
      { onSuccess: resetForm },
    );
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              Quản lý mã giảm giá cho checkout mua khóa học.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              onClick={() =>
                void Promise.all([couponsQuery.refetch(), statsQuery.refetch()])
              }
              disabled={couponsQuery.isFetching || statsQuery.isFetching}
              className="gap-2"
              title="Làm mới danh sách và thống kê coupon"
            >
              <RefreshCw
                className={`h-4 w-4 ${couponsQuery.isFetching || statsQuery.isFetching ? "animate-spin" : ""}`}
              />{" "}
              Làm mới
            </Button>
        ...
        <section className="grid gap-3 md:grid-cols-4">
          {summaryCards.map((card) => (
            <div
              key={card.label}
              className="border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900"
            >
              ...
              <div className="mt-1">
                <Select
                  ...
                  onValueChange={(event) =>
                    setForm({
                      ...form,
                      type: event as CouponType,
                      maxDiscountAmount:
                        event === "FIXED"
                          ? null
                          : form.maxDiscountAmount,
                      value:
                        event === "PERCENT" && form.value > 100
                          ? 100
                          : form.value,
                    })
                  }
>
            ...
            <label className="block text-sm font-medium">
              ...
              <Input
                ...
                placeholder={
                  form.type === "FIXED" ? "Không khả dụng" : "Không giới hạn"
                }
              />
            </label>
            <label className="block text-sm font-medium">
            ...
            <label className="block text-sm font-medium">
              ...
              <Input
                ...
                onChange={(event) =>
                  setForm({ ...form, perUserLimit: Number(event.target.value) })
                }
              />
            </label>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        variant="outline"
                        className={cn(
                          "w-full justify-start text-left font-normal h-10 border border-input bg-transparent px-3 py-2 pr-10 text-sm shadow-sm transition-colors hover:translate-y-0 active:scale-100",
                          !form[field] && "text-muted-foreground",
                        )}
                      >
                        <CalendarIcon className="mr-2 h-4 w-4 text-zinc-500" />
                        {formatDatePickerDate(form[field])}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-3" align="start">
                      <Calendar
                        selected={
                          form[field] ? new Date(form[field]!) : undefined
                        }
                        onSelect={(date) =>
            ...
            <div className="pt-7">
              ...
              <button
                ...
                className={cn(
                  ...
                  form.isActive ? STATUS_CLASS.ACTIVE : STATUS_CLASS.INACTIVE,
                )}
              >
...
>
                ...
                <SelectContent>
                  <SelectItem value="">Tất cả trạng thái</SelectItem>
                                  ...
                                  <SelectItem value="SCHEDULED">Sắp mở</SelectItem>
...
>
                ...
                <SelectContent>
                  <SelectItem value="newest">Mới nhất</SelectItem>
                                  ...
                                  <SelectItem value="expires_soon">Sắp hết hạn</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          {couponsQuery.isLoading ? (
            ...
            <div className="overflow-x-auto">
              <table
                className={`w-full text-sm transition-opacity duration-150 ${couponsQuery.isFetching ? "opacity-70" : "opacity-100"}`}
              >
                <thead>
                  <tr
                    className={`border-b text-left text-zinc-500 transition-colors ${selectedIds.length > 0 ? "bg-indigo-50/50 dark:bg-indigo-950/20" : ""}`}
                  >
                    ...
                    {selectedIds.length > 0 ? (
                      <th
                        colSpan={9}
                        className="px-4 py-2 align-middle text-left"
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() =>
                                handleExecuteMultiAction("activate")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleExecuteMultiAction("deactivate")
                              }
                      ...
                      >
                        ...
                        <td>
                          {coupon.minOrderAmount
                            ? `Từ ${money(coupon.minOrderAmount)}`
                            : "Không"}
                        </td>
                        <td>
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent>
                              ...
                              <p className="text-zinc-500">
                                {coupon.createdByName || "Không rõ tên"}
                              </p>
                              {coupon.createdBy && (
                        ...
                        <td className="text-right whitespace-nowrap">
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent>Chỉnh sửa coupon</TooltipContent>
                          </Tooltip>
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent>Xóa coupon</TooltipContent>
                          </Tooltip>
                        </td>
                      </tr>
                    );
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold">
                  Lịch sử dùng {historyCoupon.code}
                </h2>
                <p className="text-sm text-zinc-500">
                  Chỉ tính transaction đã thanh toán thành công.
                </p>
              </div>
              <Button
                variant="ghost"
                size="icon"
                onClick={() => setHistoryCoupon(null)}
              >
                <X className="h-4 w-4" />
              </Button>
            </div>
            {redemptionsQuery.isLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(redemptionsQuery.data?.redemptions ?? []).map((item) => (
                      <tr
                        key={item._id}
                ...
                {(redemptionsQuery.data?.redemptions ?? []).length === 0 && (
                  <div className="py-8 text-center text-sm text-zinc-500">
                    Coupon này chưa có lượt sử dụng.
                  </div>
                )}
              </div>
            )}
