...
export function CourseInstructor({ instructorId, instructorName, enrollmentCount, avatarUrl, bio }: Props) {
  ...
  const { data: publicProfile } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (response.status === 'ERR' || !response.data) {
        throw new Error(response.message || 'Không thể tải thông tin tác giả khóa học.');
      }
  ...
  return (
    <section className="rounded-lg border border-border bg-card p-6 lg:p-7 shadow-sm">
      <h2 className="text-2xl font-bold font-serif mb-6">Tác giả khóa học</h2>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm text-muted-foreground mb-4">Người giảng dạy</p>
          <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <UserRoundCheck className="w-4 h-4 text-primary" />
              Người giảng dạy đã được xác minh
            </span>
            <span className="flex items-center gap-1.5">
